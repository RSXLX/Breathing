...
try{
 ...
 await page.locator('.app-tabbar').getByRole('tab',{name:'作品',exact:true}).click();await page.locator('#library').waitFor({state:'visible'});check('library shows exported work',await page.locator('#work-list .card').count()===1);await page.screenshot({path:join(out,'app-library-mobile.png')});
 await page.reload();await page.locator('#library').waitFor({state:'visible'});check('app deep link and local work survive reload',await page.locator('#work-list .card').count()===1);
